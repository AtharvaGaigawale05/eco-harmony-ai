...
export const Route = createFileRoute("/challenges")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Take on weekly eco challenges, earn XP, and unlock sustainability achievements.",
      },
    ],
  }),
...
function ChallengesPage() {
  ...
  function toggleComplete(id: string) {
    setState((prev) => {
      ...
      const next: ChallengeState = {
        ...prev,
        [id]: { completed: !cur.completed, progress: !cur.completed ? 100 : 0 },
      };
      saveChallenges(next);
  ...
  function setProgress(id: string, value: number) {
    setState((prev) => {
      ...
      const next: ChallengeState = {
        ...prev,
        [id]: { completed, progress: Math.min(100, Math.max(0, value)) },
      };
      saveChallenges(next);
      return next;
    });
      ...
      <div className="mx-auto max-w-6xl px-4 py-10 space-y-6">
        <header>
          ...
          <p className="mt-2 text-muted-foreground">
            Build habits with weekly missions. Earn XP, unlock badges.
          </p>
        </header>
            ...
            return (
              <li key={c.id} className="glass rounded-2xl p-5 shadow-elegant flex flex-col">
                <div className="flex items-start justify-between">
                  ...
                  <span className="rounded-full bg-primary/15 text-primary text-xs px-2 py-0.5">
                    +{c.xp} XP
                  </span>
                </div>
                ...
                <div className="mt-4">
                  <div className="flex justify-between text-[11px] text-muted-foreground mb-1">
                    <span>Progress</span>
                    <span>{cur.progress}%</span>
                  </div>
                  <div className="h-2 rounded-full bg-secondary/60 overflow-hidden">
                    <div
                      className="h-full bg-gradient-eco transition-all"
                      style={{ width: `${cur.progress}%` }}
                    />
                  </div>
                  <input
                    type="range"
          ...
          <ul className="mt-4 grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {BADGES.map((b) => {
              ...
              return (
                <li
                  key={b.id}
                  className={`rounded-xl p-4 border ${isUnlocked ? "border-primary/40 bg-primary/10" : "border-border bg-secondary/30 opacity-70"}`}
                >
                  <div className="text-sm font-medium">{b.name}</div>
                  <div className="text-xs text-muted-foreground">{b.description}</div>
                  <div className="text-[11px] mt-1 text-primary">
                    {isUnlocked ? "Unlocked" : `Complete ${b.threshold} challenges`}
                  </div>
                </li>
              );
            })}
