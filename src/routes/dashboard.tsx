  ...
  head: () => ({
    meta: [
      { title: "Dashboard — EcoTrack AI" },
      {
        name: "description",
        content:
          "Track trends, eco score, and AI-driven recommendations on your sustainability dashboard.",
      },
    ],
  }),
...
function DashboardPage() {
  ...
  useEffect(() => {
    ...
    ecoRecommendations({
      ...
      .then((r) => {
        if (!cancelled) setRecs(r.recommendations as Rec[]);
      })
      .catch(() => {})
      .finally(() => {
  ...
  const monthlyAvg = useMemo(() => {
    ...
    return Math.round(
      history.slice(0, 4).reduce((s, h) => s + h.totalKgPerYear, 0) /
      ...
      <AppShell>
        <div className="mx-auto max-w-3xl px-4 py-20 text-center">
          ...
          <p className="mt-2 text-muted-foreground">
            Save your first footprint snapshot to unlock the dashboard.
          </p>
          <Link
            to="/calculator"
            className="mt-6 inline-flex items-center gap-2 rounded-xl bg-gradient-eco px-5 py-3 text-sm font-medium text-primary-foreground shadow-glow"
          >
    ...
    <AppShell>
      <div className="mx-auto max-w-6xl px-4 py-10 space-y-6">
        <header>
          ...
          <p className="mt-2 text-muted-foreground">
            Live snapshot, trends and personalized AI insights.
          </p>
        </header>
        ...
        <div className="grid lg:grid-cols-3 gap-4">
          <div className="glass rounded-2xl p-6 shadow-elegant">
            ...
            <div className="mt-1 text-3xl font-semibold">
              {Math.round(latest.totalKgPerYear / 365)}{" "}
              <span className="text-base text-muted-foreground">kg/day</span>
            </div>
            <div className="text-xs text-muted-foreground mt-1">
              {latest.totalKgPerYear.toLocaleString()} kg / year
            </div>
          </div>
          <div className="glass rounded-2xl p-6 shadow-elegant">
            <div className="text-xs uppercase tracking-widest text-muted-foreground">
            ...
            <div className="mt-1 text-3xl font-semibold">
              {monthlyAvg} <span className="text-base text-muted-foreground">kg/month</span>
            </div>
            <div className="text-xs text-muted-foreground mt-1">
              Across last {Math.min(4, history.length)} snapshots
            </div>
          </div>
        ...
        <div className="grid lg:grid-cols-3 gap-4">
          <div className="lg:col-span-2 glass rounded-2xl p-6 shadow-elegant">
            <div className="flex items-center justify-between">
              <h2 className="font-semibold flex items-center gap-2">
                <Sparkles className="size-4 text-primary" aria-hidden /> AI recommendations
              </h2>
              ...
              {recs.map((r, i) => (
                <li key={i} className="rounded-xl bg-secondary/40 p-4 border border-border">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[10px] uppercase tracking-widest rounded-full px-2 py-0.5 bg-primary/15 text-primary">
                      {r.impact}
                    </span>
                  </div>
          ...
          <div className="glass rounded-2xl p-6 shadow-elegant">
            <h2 className="font-semibold flex items-center gap-2">
              <Trophy className="size-4 text-primary" aria-hidden /> Achievements
            </h2>
            <ul className="mt-4 space-y-2">
              {BADGES.map((b) => {
                ...
                return (
                  <li
                    key={b.id}
                    className={`rounded-lg p-3 border ${unlocked ? "border-primary/40 bg-primary/10" : "border-border bg-secondary/30 opacity-70"}`}
                  >
