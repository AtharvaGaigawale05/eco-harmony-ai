...
export const Route = createFileRoute("/assistant")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Chat with your AI sustainability coach for personalized eco-friendly advice." },
    ],
  }),
...
function AssistantPage() {
  const [messages, setMessages] = useState<Msg[]>([
    { id: "welcome", role: "assistant", content: "Hi 👋 I'm your EcoTrack AI coach. Ask me anything about reducing your carbon footprint — I'll personalize advice using your latest data." },
  ]);
      ...
      <div className="mx-auto max-w-3xl px-4 py-8 flex flex-col h-[calc(100dvh-8rem)]">
        <header className="mb-4">
          <h1 className="text-2xl sm:text-3xl font-semibold tracking-tight flex items-center gap-2">
            <span className="inline-flex size-9 items-center justify-center rounded-xl bg-gradient-eco shadow-glow"><Bot className="size-5 text-primary-foreground" aria-hidden /></span>
            AI Sustainability Coach
          </h1>
          <p className="mt-1 text-sm text-muted-foreground">Personalized, motivational guidance for a lighter footprint.</p>
        </header>

        <div ref={scrollerRef} className="flex-1 overflow-y-auto glass rounded-2xl p-4 sm:p-6 shadow-elegant space-y-4" aria-live="polite" aria-busy={loading}>
          {messages.map((m) => (
            <div key={m.id} className={`flex gap-3 ${m.role === "user" ? "justify-end" : ""}`}>
              {m.role === "assistant" && (
                <span className="shrink-0 size-8 rounded-lg bg-primary/15 text-primary grid place-items-center"><Bot className="size-4" aria-hidden /></span>
              )}
              ...
              {m.role === "user" && (
                <span className="shrink-0 size-8 rounded-lg bg-secondary grid place-items-center"><User className="size-4" aria-hidden /></span>
              )}
            </div>
          ))}
          {loading && (
        ...
        {messages.length <= 1 && (
          <div className="mt-3 flex flex-wrap gap-2">
            {STARTERS.map((s) => (
              <button key={s} type="button" onClick={() => send(s)} className="rounded-full glass px-3 py-1.5 text-xs hover:bg-secondary/60 inline-flex items-center gap-1.5">
        ...
        >
          <label className="sr-only" htmlFor="chat-input">Ask your AI coach</label>
          <textarea
            id="chat-input"
            ref={inputRef}
            rows={1}
